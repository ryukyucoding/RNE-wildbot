...
class Nav2Processing:
    ...
    def camera_nav_pid_command(
        ...
        if not detected:
            ...
            # EMA 濾波：平滑 depth 跳動
            if depth > 0.0:
                alpha_d = max(0.05, min(1.0, float(depth_ema_alpha)))
                if self._vs_filtered_depth is None:
                    self._vs_filtered_depth = depth
                else:
                    self._vs_filtered_depth = alpha_d * depth + (1.0 - alpha_d) * self._vs_filtered_depth
                depth = self._vs_filtered_depth
